...
const mockRcloneTools = {
    getAllFilePaths: jest.fn(() => Promise.resolve({ allFilePaths: [
...
const mockDetectionService = {
    generateReport: jest.fn(() => Promise.resolve({ 
        results: [{
            ...
            reason: "Обнаружены маркеры кода, характерные для исходных файлов (e.g., .tsx, .js)."
        }]
    }))
};
...
describe('MigrationEngine Core Logic Tests', () => {
    ...
    describe('extractAndStandardizeUid', () => {
        ...
        it('should handle deep, ambiguous paths and return fallback', () => {
            ...
            expect(engine.extractAndStandardizeUid(path)).toMatch(/^BS-UNKNOWN-[A-Z]{2}$/); 
        });
    ...
    describe('determineCategory', () => {
        it('should categorize reports correctly', () => {
            expect(engine.determineCategory('report_q1_2025.docx')).toBe('ReportArchive');
        });
    ...
    describe('runPlan', async () => {
        it('should run through all stages and generate a complete MigrationPlan', async () => {
            ...
            jest.spyOn(FileSystem, 'getFileMetadata').mockImplementation(async (path) => {
                ...
                return {
                    ...
                    modTime: '2025-01-15',
                    isDirectory: isDir,
                    rawBytes: isDir ? '' : (path.includes('code') ? 'SIMULATED_RAW_BYTES_FOR_Code' : 'SIMULATED_RAW_BYTES_for_Image')
                };
            });
            ...
            const mockGenerateReport = jest.fn(async (batch) => {
                ...
                if (file.rawBytes.includes('Code')) {
                    return { results: [{ classification: "Code", confidenceScore: 0.95, reason: "Code" }] };
                } else if (file.rawBytes.includes('Image')) {
                    return { results: [{ classification: "Image", confidenceScore: 0.85, reason: "Image" }] };
                } else if (file.rawBytes.includes('dump')) {
                    return { results: [{ classification: "RawData", confidenceScore: 0.9, reason: "Raw" }] };
                } else if (file.rawBytes.includes('report')) {
                     return { results: [{ classification: "Document", confidenceScore: 0.9, reason: "Document" }] };
                } else {
                    return { results: [] };
                }
            });
